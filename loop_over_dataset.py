...
def make_video():
    out = cv2.VideoWriter('output.avi', cv2.VideoWriter_fourcc('M','J','P','G'), 8, (frame_width,frame_height))
    # Read and display the images
    cnt_frame = 0
    for image in images_avi:
        print('Recording frame #' + str(cnt_frame))
        #image = cv2.resize(image, (frame_width/2, frame_height))
        out.write(image) # Write the image to the video
        cv2.imshow('range_image', image)
        if cv2.waitKey(vis_pause_time) == ord('q'): # Hit `q` to exit
            break
...
while True:
    try:
        ...
        if 'show_range_image' in exec_list:
            ...
            frame_width = img_range.shape[1]
            frame_height = img_range.shape[0]
            img_range_aux = cv2.cvtColor(img_range , cv2.COLOR_GRAY2BGR)
            images_avi.append(img_range_aux)
