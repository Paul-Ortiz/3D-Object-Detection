...
def show_range_image(frame, lidar_name):
    ...
    ri, camera_projection, range_image_pose = waymo_utils.parse_range_image_and_camera_projection(lidar)  # Parse the top laser range image and get the associated projection.
    ...
    print (ri_center-deg90, ri_center+deg90)
    ri_range = ri_range[:, np.arange(ri_center-deg90, ri_center+deg90-2, 1)]
    # step 5 : map the intensity channel onto an 8-bit scale and normalize with the difference between the 1- and 99-percentile to mitigate the influence of outliers
    perc_1, perc_99 = np.percentile(ri_intensity, (1, 99))
    ri_intensity[ri_intensity < perc_1] = perc_1
    ri_intensity[ri_intensity > perc_99] = perc_99
    ri_intensity = 255 * (ri_intensity - perc_1) / (perc_99 - perc_1)
    ri_intensity = ri_intensity.astype(np.uint8)

    deg90 = int(ri_intensity.shape[1] / 4)
    ri_center = int(ri_intensity.shape[1]/2)
    ri_intensity = ri_intensity[:, np.arange(ri_center-deg90, ri_center+deg90-2, 1)]
